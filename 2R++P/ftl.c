...
// 페이지에 비트추가 기본값 0
// 콜드에서 write 발생하면 비트 1할당 region2에서 카피백될 때 비트 값 1이면 2로 세팅 2에서 카피백 발생하면 콜드 리전으로
// region2에서 1이나 2일 때 write 발생하면 비트 값 1로 만들고 region1로
// 콜드에서 write 발생했는데 비트값 2면 3으로 세팅 위와 동일하게 동작해서 4, 5까지 도달하면 콜드 리전으로
...
static void ssd_init_lines(struct ssd *ssd)
{
    ...
    //default value 0.5, 0.7
    //졸업 논문 추가 실험 위해 0.2 ~ 0.7 정도까지 시도
    tr_info->count = 0;
}
...
static void ssd_advance_write_pointer_gc(struct ssd *ssd, int cold, int pgc)
{
    struct ssdparams *spp = &ssd->sp;
    struct tr_mgmt *tr_info = &ssd->tr_info;
...
static struct ppa get_new_page_gc(struct ssd *ssd, int cold, int pgc)
{
    struct write_pointer *wpp = &ssd->cp;
    struct ppa ppa;
    int block_cold = 2;
...
static uint64_t gc_write_page(struct ssd *ssd, struct ppa *old_ppa, int cold)
{
    ...
    pg = get_pg(ssd, old_ppa);
    pgc = pg->cold;
    pgc2 = pgc;

    ftl_assert(valid_lpn(ssd, lpn));
    new_ppa = get_new_page_gc(ssd, cold, pgc);
    /* update maptbl */
    ...
    ssd_advance_write_pointer_gc(ssd, cold, pgc);
        ...
        while(1){
            ...
            if(i == NULL || i->vline->id == fifo_tail){
                if(cold_block_valid_ratio < 1){
                    cold_block_valid_ratio += 0.04;
                }

                if(frozen_block_valid_ratio < 1){
                    frozen_block_valid_ratio += 0.04;
                }
